...
LastEditTime: 2023-03-07 17:50:02
...
for idx, row in df.iterrows():
    ...
    for col in range(start_idx, end_idx):
        ...
        for i, task in enumerate(tasks):
            ...
            tasksx.append(tag)
            if len(remark) > 0:
                remarksx.append(remark)
        print(idx, row[0], timeline[col], "tag:", ','.join(tasksx), "remark:", '；'.join(remarksx), avg_work_time)
        data.append([row[0], timeline[col], ','.join(tasksx), '；'.join(remarksx), avg_work_time])
df2 = pd.DataFrame(columns=["姓名", "日期", "参与项目", "备注", "平均工时"])
for i, d in enumerate(data):
    df2.loc[i] = d

output_name = "ups2_"+start_date.strftime("%Y%m%d")+"-"+end_date.strftime("%Y%m%d")+".xlsx"
